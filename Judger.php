...
class Judger extends Curl
{
    ...
    public function judge($row)
    {
        try {
            $sub=[];
            $response=$curl->grab_page("https://pintia.cn/api/submissions/".$row['remote_id'], 'pta', ['Accept: application/json;charset=UTF-8'], $this->judgerModel->detail($row['jid'])['handle']);
